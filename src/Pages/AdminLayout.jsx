...
const AdminDashboard = () => {
  ...
  React.useEffect(() => {
    ...
    const getData = async(token)=>{
      ...
      if(user_id){
        ...
        if(user.data.user.isAdmin === false || user.data.user.access_valid === false)
  ...
  React.useEffect(() => {
    ...
    if (!component) {
      setComponent(
        adminDashboardRoutes.filter((route) => route.path === "/admin")[0].component
      );
    } else {
  ...
  React.useEffect(() => {
    console.log(component);
    if (component === null) {
      let c = adminDashboardRoutes.filter((route) => route.path === '/admin');
